SELECT * FROM `promising-idea-486716-m2.Project.rides` LIMIT 1000

SELECT * FROM `promising-idea-486716-m2.Project.stations` LIMIT 1000

SELECT * FROM `promising-idea-486716-m2.Project.users` LIMIT 1000


-- get count of rows per table
SELECT 
  (SELECT COUNT(*) FROM `promising-idea-486716-m2.Project.rides`) AS total_rides,
  (SELECT COUNT(*) FROM `promising-idea-486716-m2.Project.stations`) AS total_stations,
  (SELECT COUNT(*) FROM `promising-idea-486716-m2.Project.users`) AS total_users
...
FROM `promising-idea-486716-m2.Project.rides`
...
FROM `promising-idea-486716-m2.Project.rides` AS r
JOIN `promising-idea-486716-m2.Project.users` AS u
...
FROM `promising-idea-486716-m2.Project.rides` AS r
JOIN `promising-idea-486716-m2.Project.stations` AS s
...
-- Categorizing rides into, Medium and Long
SELECT
    CASE
        WHEN TIMESTAMP_DIFF(end_time, start_time, MINUTE) <=10 THEN 'Short (<10m)'
        WHEN TIMESTAMP_DIFF(end_time, start_time, MINUTE) BETWEEN 11 AND 30 THEN 'Medium (11-30)'
        ELSE 'Long (>30m)'
...
FROM `promising-idea-486716-m2.Project.stations` AS s
JOIN departure d ON s.station_id = d.start_station_id
...
WITH monthly_signups AS (
  ...
  FROM `promising-idea-486716-m2.Project.users`
...
--- daily count moving average in one week
